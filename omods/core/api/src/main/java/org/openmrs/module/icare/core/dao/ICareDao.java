...
public class ICareDao extends BaseDAO<Item> {
	...
	public List<Visit> getVisitsByOrderType(String search, String orderTypeUuid, String locationUuid,
		...
		Query query = null;
		DbSession session = this.getSession();
		String queryStr = "SELECT distinct v FROM Visit v ,Invoice invoice" + " INNER JOIN v.patient p" + " INNER JOIN p.names pname";
...
//				        + " HAVING SUM(item.price*item.quantity) <= (SUM(pi.amount) + SUM(di.amount))))";
				queryStr += " AND v.id = invoice.visit.id "+ "AND (SELECT SUM(item.price*item.quantity) FROM InvoiceItem item WHERE item.id.invoice = invoice) \n"
